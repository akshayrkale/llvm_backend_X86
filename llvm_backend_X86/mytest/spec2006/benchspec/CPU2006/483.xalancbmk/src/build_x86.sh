...
TOOLS_PATH=~/llvm523/llvm_backend/build_x86/Debug+Asserts/bin
for c in *.cpp; do
     if [ -f $c.o ]
     then
         continue
    ...
    $TOOLS_PATH/clang++ -Wno-main-return-type -emit-llvm -DENABLE_SCORE -DENABLE_PREVIEW -DENABLE_HIGH_SCORE -O3 -S -c -o $c.bc -nobuiltininc -isystem $LIBC_PATH/include $c -isystem .
    echo "[Running] llc $c.bc"
    $TOOLS_PATH/llc -soft-float -march=x86-64 -O3 $c.bc -o $c.S
