//===-- Cse523MachObjectWriter.cpp - Cse523 Mach-O Writer -----------------------===//
//
...
namespace {
class Cse523MachObjectWriter : public MCMachObjectTargetWriter {
  ...
  void RecordCse523Relocation(MachObjectWriter *Writer,
                              const MCAssembler &Asm,
                              const MCAsmLayout &Layout,
                              const MCFragment *Fragment,
                              const MCFixup &Fixup,
                              MCValue Target,
                              uint64_t &FixedValue);
  void RecordCse523_64Relocation(MachObjectWriter *Writer,
                              const MCAssembler &Asm,
                              const MCAsmLayout &Layout,
                              const MCFragment *Fragment,
                              const MCFixup &Fixup,
                              ...
                              uint64_t &FixedValue);
public:
  Cse523MachObjectWriter(bool Is64Bit, uint32_t CPUType,
                      uint32_t CPUSubtype)
  ...
  void RecordRelocation(MachObjectWriter *Writer,
    ...
    if (Writer->is64Bit())
      RecordCse523_64Relocation(Writer, Asm, Layout, Fragment, Fixup, Target,
                             FixedValue);
    else
      RecordCse523Relocation(Writer, Asm, Layout, Fragment, Fixup, Target,
                          FixedValue);
  }
};
}
...
static bool isFixupKindRIPRel(unsigned Kind) {
  return Kind == Cse523::reloc_riprel_4byte ||
    Kind == Cse523::reloc_riprel_4byte_movq_load;
}
...
void Cse523MachObjectWriter::RecordCse523_64Relocation(MachObjectWriter *Writer,
                                                 const MCAssembler &Asm,
                                                 const MCAsmLayout &Layout,
                                                 const MCFragment *Fragment,
                                                 const MCFixup &Fixup,
  ...
  if (IsPCRel) {
    // Compensate for the relocation offset, Darwin cse523_64 relocations only have
  ...
  if (Target.isAbsolute()) { // constant
    ...
    Type = MachO::CSE523_RELOC_UNSIGNED;
    Index = 0;
    ...
    if (IsPCRel) {
      IsExtern = 1;
      Type = MachO::CSE523_RELOC_BRANCH;
    }
    ...
    Type = MachO::CSE523_RELOC_SUBTRACTOR;
  } else {
    ...
    // understand cse523_64 relocation entries, and expects to find values that
    ...
    // cse523_64 almost always uses external relocations, except when there is no
    ...
    if (IsPCRel) {
      if (IsRIPRel) {
        if (Modifier == MCSymbolRefExpr::VK_GOTPCREL) {
          // cse523_64 distinguishes movq foo@GOTPCREL so that the linker can
          // rewrite the movq to an leaq at link time if the symbol ends up in
          // the same linkage unit.
          if (unsigned(Fixup.getKind()) == Cse523::reloc_riprel_4byte_movq_load)
            Type = MachO::CSE523_RELOC_GOT_LOAD;
          else
            Type = MachO::CSE523_RELOC_GOT;
        }  else if (Modifier == MCSymbolRefExpr::VK_TLVP) {
          Type = MachO::CSE523_RELOC_TLV;
        }  else if (Modifier != MCSymbolRefExpr::VK_None) {
          report_fatal_error("unsupported symbol modifier in relocation",
                             false);
        } else {
          Type = MachO::CSE523_RELOC_SIGNED;

          // The Darwin cse523_64 relocation format has a problem where it cannot
          // encode an address (L<foo> + <constant>) which is outside the atom
          // containing L<foo>. Generally, this shouldn't occur but it does
          // happen when we have a RIPrel instruction with data following the
          // relocation entry (e.g., movb $012, L0(%rip)). Even with the PCrel
          // adjustment Darwin cse523_64 uses, the offset is still negative and the
          ...
          case 1: Type = MachO::CSE523_RELOC_SIGNED_1; break;
          case 2: Type = MachO::CSE523_RELOC_SIGNED_2; break;
          case 4: Type = MachO::CSE523_RELOC_SIGNED_4; break;
          }
        }
      } else {
      ...
      if (Modifier == MCSymbolRefExpr::VK_GOT) {
        ...
        if (Kind == Cse523::reloc_signed_4byte)
  ...
  // CSE523_64 always writes custom values into the fixups.
  FixedValue = Value;
...
bool Cse523MachObjectWriter::RecordScatteredRelocation(MachObjectWriter *Writer,
                                                    const MCAssembler &Asm,
                                                    const MCAsmLayout &Layout,
                                                    const MCFragment *Fragment,
                                                    const MCFixup &Fixup,
...
void Cse523MachObjectWriter::RecordTLVPRelocation(MachObjectWriter *Writer,
                                               const MCAssembler &Asm,
                                               const MCAsmLayout &Layout,
                                               const MCFragment *Fragment,
                                               const MCFixup &Fixup,
...
void Cse523MachObjectWriter::RecordCse523Relocation(MachObjectWriter *Writer,
                                              const MCAssembler &Asm,
                                              const MCAsmLayout &Layout,
                                              const MCFragment *Fragment,
                                              const MCFixup &Fixup,
...
MCObjectWriter *llvm::createCse523MachObjectWriter(raw_ostream &OS,
                                                bool Is64Bit,
                                                uint32_t CPUType,
                                                uint32_t CPUSubtype) {
  return createMachObjectWriter(new Cse523MachObjectWriter(Is64Bit,
                                                        CPUType,
                                                        CPUSubtype),
