//===-- Cse523TargetTransformInfo.cpp - Cse523 specific TTI pass ----------------===//
//
...
/// Cse523 target machine. It uses the target's detailed information to provide
/// more precise answers to certain TTI queries, while letting the target
/// independent and default TTI implementations handle the rest.
///
//===----------------------------------------------------------------------===//

#define DEBUG_TYPE "cse523tti"
#include "Cse523.h"
...
namespace llvm {
void initializeCse523TTIPass(PassRegistry &);
}

namespace {

class Cse523TTI LLVM_FINAL : public ImmutablePass, public TargetTransformInfo {
  const Cse523Subtarget *ST;
...
public:
  Cse523TTI() : ImmutablePass(ID), ST(0), TLI(0) {
    llvm_unreachable("This pass cannot be directly constructed");
  }

  Cse523TTI(const Cse523TargetMachine *TM)
    : ImmutablePass(ID), ST(TM->getSubtargetImpl()),
      TLI(TM->getTargetLowering()) {
    initializeCse523TTIPass(*PassRegistry::getPassRegistry());
  }
...
INITIALIZE_AG_PASS(Cse523TTI, TargetTransformInfo, "cse523tti",
                   "Cse523 Target Transform Info", true, true, false)
char Cse523TTI::ID = 0;

ImmutablePass *
llvm::createCse523TargetTransformInfoPass(const Cse523TargetMachine *TM) {
  return new Cse523TTI(TM);
}
...
Cse523TTI::PopcntSupportKind Cse523TTI::getPopcntSupport(unsigned TyWidth) const {
...
unsigned Cse523TTI::getNumberOfRegisters(bool Vector) const {
...
unsigned Cse523TTI::getRegisterBitWidth(bool Vector) const {
  if (Vector) {
...
unsigned Cse523TTI::getMaximumUnrollFactor() const {
  if (ST->isAtom())
    return 1;
...
unsigned Cse523TTI::getArithmeticInstrCost(unsigned Opcode, Type *Ty,
...
unsigned Cse523TTI::getShuffleCost(ShuffleKind Kind, Type *Tp, int Index,
                                Type *SubTp) const {
...
unsigned Cse523TTI::getCastInstrCost(unsigned Opcode, Type *Dst, Type *Src) const {
  int ISD = TLI->InstructionOpcodeToISD(Opcode);
  assert(ISD && "Invalid opcode");
...
unsigned Cse523TTI::getCmpSelInstrCost(unsigned Opcode, Type *ValTy,
                                    Type *CondTy) const {
  // Legalize the type.
...
unsigned Cse523TTI::getVectorInstrCost(unsigned Opcode, Type *Val,
...
unsigned Cse523TTI::getScalarizationOverhead(Type *Ty, bool Insert,
                                            bool Extract) const {
...
unsigned Cse523TTI::getMemoryOpCost(unsigned Opcode, Type *Src, unsigned Alignment,
...
unsigned Cse523TTI::getAddressComputationCost(Type *Ty, bool IsComplex) const {
...
unsigned Cse523TTI::getReductionCost(unsigned Opcode, Type *ValTy,
...
unsigned Cse523TTI::getIntImmCost(const APInt &Imm, Type *Ty) const {
...
unsigned Cse523TTI::getIntImmCost(unsigned Opcode, const APInt &Imm,
                               Type *Ty) const {
  assert(Ty->isIntegerTy());
  ...
  case Instruction::ICmp:
    if (Imm.getBitWidth() <= 64 && isInt<32>(Imm.getSExtValue()))
      ...
      return Cse523TTI::getIntImmCost(Imm, Ty);
...
unsigned Cse523TTI::getIntImmCost(Intrinsic::ID IID, const APInt &Imm,
                               Type *Ty) const {
  assert(Ty->isIntegerTy());
